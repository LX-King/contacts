(function() {
	...
	/**
	 * [animate to generate 16 random number]
	 * @return {[String]}
	 */
	FC.guid = function() {
	...
	/*var Events = FC.Events = {
		bind: function(event, func) {
			...
			if (!event) return;
			if (!func) return;
			for (var f in func)((callBacks[event]) || (callBacks[event] = [])).push(f);
			return this;
		},
		unbind: function(event, func) {
			if (!event) return;
			if (!func) {
				var len = this._callbacks[event].length;
				this._callbacks[event].slice(0, len);
				return this;
			}
		},
		trigger: function() {
			var args = FC.toArray(arguments);
			var e = args.shift();
			if (!e) return;
			if (!this._callbacks[e]) return;
			for (var i = 0; i < this._callbacks[e].length; i++) {
				if (this._callbacks[i].apply(this, args) === false) return false;
			}
			return this;
		}
	};*/
	...
	/*var Class = FC.Class = {
		KEYWORDS: ["extend", "extended", "include", "included"],
		created: function() {},
		inherited: function() {},
		prototype: {
			init: function() {}
		...
		create: function(include, extend) {
			...
			klass.fn = klass.prototype = Object.create(this.prototype);
		...
		include: function(o) {
			...
			instance.fn = instance.prototype = Object.create(this.prototype);
			for (var p in o) {
				if (this.KEYWORDS.indexOf(p) == -1)
					this.fn[p] = o[p];
			}
		...
		proxyAll: function(funcs) {
			var funcsArr = FC.toArray(funcs);
			for (var i = 0; i < funcsArr.length; i++) {
				this[funcsArr[i]] = this.proxy(funcsArr[i]);
			}
		}
	};
...
*/
  ...
  var Class = FC.Class = {
    ...
    proxyAll: function(){
      var functions = FC.toArray(arguments);
  ...
  Class.inst               = Class.init;
	...
	// Module Model
	// ================================================================================
	var Model = FC.Model = Class.create();
	Model.extend(FC.Events);
	Model.createSub = Model.create;
	Model.setup = function(Name, props) {
		var model = Model.createSub(this);
		if (name) model.name = name;
		if (props) model.attributes = props;
		return model;
	};
	...
	// Module Controller
	var Controller = FC.Controller = FC.Class.create();
	...
	// Module App
	FC.App = Controller.create({
		create:function(props){
			this.parent.include(props);
			return this;
		}
	}).init();
	Controller.fn.App = FC.App;
